...
class Thermocouple(object):
    ...
    def c2e(self, Tc):
        ...
        if self.type is 'K':
            ...
            E += 0.1185976 * math.exp(-0.1183432e-3 * (Tc - 0.1269686e3)^2)

        return E

    def e2c(self, E):
        """Convert reading in mV to reading in C for Thermocouple type.

        @param E: Reading in mV
        @type E: float
        @return: Reading in C
        @rtype: float
        """
        ...
        # Gets index for coefs for which the NIST function is valid
        e_r = 0
        while self.e2c_ranges[self.type][e_r] <= E and e_r < len(self.e2c_ranges[self.type]):
            e_r += 1

        # Recursive formulation of polynomial coefficient solution.
        for a in range(len(self.e2c_coefs[self.type][e_r])-1,0):
            Tc = self.e2c_coefs[self.type][e_r][a] + E * Tc
...
class TemperatureProbe(object):
    ...
    def __init__(self, type, read_fn, read_cj_fn=None, tc_type=None, thermis_fn=None):
        """

        @param type: Probe type, recognizes 'RTD', 'Thermistor', 'Thermocouple'
        @type type:  str
        @param read_fn:  Function which returns the ADC voltage.
        @type read_fn: function
        @param read_cj_fn: Function to obtain the cold junction temperature
        @type read_cj_fn: function
        @param tc_type: Thermocouple type: 'K', etc.
        @type tc_type: str
        @param thermis_fn: Function to convert thermistor voltage to temperature
        """
    ...
    def reading2temp(self, R_T):
        """Stub for converting resistance value to temperature.
